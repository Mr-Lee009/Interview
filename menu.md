...
- `Task/sumolation/microservice/QA.md`: 10 câu hỏi phỏng vấn Microservice kèm câu trả lời, ví dụ thực tế và hướng xử lý các vấn đề production như Saga, service discovery, API Gateway, circuit breaker, tracing và idempotency.
...
- `Task/sumolation/kafka/doc.md`: tài liệu Kafka đầy đủ cho Java Developer/Senior.
- `Task/sumolation/kafka/bank-fraud-detection.md`: case study Kafka cho phát hiện giao dịch nghi ngờ trong ngân hàng.
- `Task/sumolation/kafka/bank-fraud-detection.html`: demo mô phỏng flow fraud detection banking bằng Kafka.
