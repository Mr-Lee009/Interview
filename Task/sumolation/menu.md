...
- `kafka-rabbitmq-cluster-performance.md`: so sánh hiệu suất Kafka Cluster và RabbitMQ Cluster khi đều có leader/master và replica/follower; giải thích throughput, latency, replication, bottleneck và cách chọn theo bài toán.
- `message-queue-performance-test/`: bộ kịch bản benchmark Kafka và RabbitMQ, gồm tài liệu test, docker compose riêng cho từng hệ thống và script chạy producer/consumer/perftest.
